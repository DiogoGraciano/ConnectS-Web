...
class usuarioDAOPgsql implements usuarioDAO{
    ...
    public function add_usuario(usuario $u){
            ...
            (cd_usuario, nm_usuario, cd_cliente, nm_terminal, nm_sistema, nm_usuario, senha, obs) 
            VALUES 
            (:cd_usuario,:nm_usuario,:cd_cliente,:nm_terminal,:nm_sistema,:nm_usuario,:senha,:obs)"); 
    ...
    public function update_usuario(usuario $u){
        ...
        ,nm_terminal = :nm_terminal, nm_sistema = :nm_sistema, nm_usuario = :nm_usuario, senha = :senha, 
    ...
    $array = [];
    $sql = $this->pdo->query('select cd_usuario,tb_usuario.cd_cliente,nm_cliente,nr_loja,nm_usuario,nm_terminal,nm_sistema,nm_usuario,senha,obs from cadastro.tb_usuario inner join cadastro.tb_cliente on tb_cliente.cd_cliente = tb_usuario.cd_cliente');
    if ($sql ->rowCount() > 0){
        $dados = $sql->fetchAll();
    ...
    $array = [];
    $sql = $this->pdo->prepare("select cd_usuario,tb_usuario.cd_cliente,nm_cliente,nr_loja,nm_usuario,nm_terminal,nm_sistema,nm_usuario,senha,obs from cadastro.tb_usuario inner join cadastro.tb_cliente on tb_cliente.cd_cliente = tb_usuario.cd_cliente where cd_usuario = :cd"); 
    $sql ->bindValue(':cd', $cd);
    $sql ->execute();
    ...
    public function findbycd_cliente($cd){
        $array = [];
        $sql = $this->pdo->prepare("select cd_usuario,tb_usuario.cd_cliente,nm_cliente,nr_loja,nm_usuario,nm_terminal,nm_sistema,nm_usuario,senha,obs from cadastro.tb_usuario inner join cadastro.tb_cliente on tb_cliente.cd_cliente = tb_usuario.cd_cliente where tb_usuario.cd_cliente = :cd"); 
        $sql ->bindValue(':cd', $cd);
        $sql ->execute();
    ...
    cd_usuario,tb_usuario.cd_cliente,nm_cliente,nr_loja,nm_usuario,nm_terminal,nm_sistema,nm_usuario,senha,obs from cadastro.tb_usuario inner join cadastro.tb_cliente on tb_cliente.cd_cliente = tb_usuario.cd_cliente 
