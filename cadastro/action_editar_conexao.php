...
$cd_conexao = filter_input(INPUT_POST, 'cd_conexao');
...
if($cd_cliente && $nm_conexao && $nm_terminal && $nm_programa && $nm_usuario && $senha && $obs){
    ...
    $c->setsenha($senha);
    $c->setobs($obs);
    $conexaoDAO -> update_conexao($c);

    header("Location: consulta_cadastro.php");
    exit;

}else{
    header("Location: editar_conexao.php?cd=<?php echo $cd_conexao;?>");
    exit;
}
